...
def main(num_of_mips: int, multi_label: bool, starting_angle: float, ending_angle: float,
         generate_ct_only: bool, input_path: str, output_path: str):

    project_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))

    # Input and Output paths
    Input_path = Path(project_dir) / Path(input_path)
    Output_path = Path(project_dir) / Path(output_path) / f'MIPs{num_of_mips}'
    ...
    print(f'Patients paths loaded from {Input_path}... Total: {len(trials_paths)}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate MIPs (SUV, SEG, CT) from PET/CT datasets")
    parser.add_argument("--num_of_mips", type=int, default=80, help="Number of MIPs to generate")
    parser.add_argument("--multi_label", action="store_true", help="Use multi-label segmentation")
    parser.add_argument("--starting_angle", type=float, default=0, help="Starting angle for MIPs")
    parser.add_argument("--ending_angle", type=float, default=180, help="Ending angle for MIPs")
    parser.add_argument("--generate_ct_only", action="store_true", help="Generate CT-only MIPs using SUV_inds")
    parser.add_argument("--input_path", type=str, required=True, help="Input dataset path")
    parser.add_argument("--output_path", type=str, required=True, help="Output path for MIPs")
    ...
    main(
        ...
        ending_angle=args.ending_angle,
        generate_ct_only=args.generate_ct_only,
