...
class PatchEmbed(nn.Module):
    ...
    def forward(
        ...
        spatial_token_size = x.shape[-3:]
        # TODO need output of x.shape BEFORE reshape
        x = x.reshape(x.shape[0], x.shape[1], -1).transpose(2, 1)
        return x, spatial_token_size
...
class Hiera(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        self.embed_dim = embed_dim  # FIXME: I ADDED!
 ...
 #TODO who is Daniel??  - add and argument to get_pos_... function
    def get_pos_embed(self, patch_dims) -> torch.Tensor:
        ...
        # )
        # self.pos_embed_temporal = nn.Parameter(
        #     torch.zeros(1, patch_dims[0], self.embed_dim)
        # )

        if self.sep_pos_embed:
            # TODO change self.pos_embed_spatial - update number of spatial tokens
            # TODO don't change the repeat
            # TODO don't change the pos_embed_temporal - but DO change the following:
            #  self.tokens_spatial_shape[1] * self.tokens_spatial_shape[2]
            return self.pos_embed_spatial.repeat(
                1, self.tokens_spatial_shape[0], 1
            )[:, :(patch_dims[1]*patch_dims[2]), :] + torch.repeat_interleave(
    ...
    def forward(
        ...
        x = x + self.get_pos_embed(patch_dims)
        x = self.unroll(x)
