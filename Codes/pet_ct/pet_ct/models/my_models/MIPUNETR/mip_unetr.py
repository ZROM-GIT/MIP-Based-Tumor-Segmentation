...
class MipUNETR(nn.Module):
    ...
    def proj_mip_img(self, x):
        return rearrange(x, 'b c h w d -> (b d) c h w')

    def proj_feat_enc(self, x):
        new_view = [x.size(0)] + self.proj_view_shape
        x = x.contiguous().view(new_view)
        x = x.permute(self.proj_axes)
        return rearrange(x, 'b c d h w -> (b d) c h w')

    def proj_feat_back(self, x, d=16):
        return rearrange(x, '(b d) c h w -> b c h w d', d=d)

    def forward(self, x_in):
        x, hidden_states_out = self.mip_vit(x_in)
        enc1 = self.encoder1(self.proj_mip_img(x_in))
        x2 = hidden_states_out[3]
        enc2 = self.encoder2(self.proj_feat_enc(x2))
        x3 = hidden_states_out[6]
        enc3 = self.encoder3(self.proj_feat_enc(x3))
        x4 = hidden_states_out[9]
        enc4 = self.encoder4(self.proj_feat_enc(x4))
        dec4 = self.proj_feat_enc(x)
        dec3 = self.decoder5(dec4, enc4)
        dec2 = self.decoder4(dec3, enc3)
        dec1 = self.decoder3(dec2, enc2)
        out = self.decoder2(dec1, enc1)  # TODO: FIX and check why this isn't working
        final_out = self.out(out)
        return self.proj_feat_back(final_out, d=self.feat_size[-1])
