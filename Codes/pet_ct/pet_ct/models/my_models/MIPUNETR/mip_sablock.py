...
class MipSABlock(nn.Module):
    ...
    def __init__(
        ...
        include_mip_dim: bool = True,  # Include MIP dimension
        ...
        self.include_mip_dim = include_mip_dim
        ...
        if use_combined_linear:
            ...
            if include_mip_dim:
                self.input_rearrange = Rearrange("m b h (qkv l d) ->m qkv b l h d", qkv=3, l=num_heads)
            else:
        ...
        if include_mip_dim:
            self.out_rearrange = Rearrange("m b l h d ->m b h (l d)")
        else:
    ...
    def forward(self, x):
        ...
        if self.use_flash_attention:
            ...
            att_mat = torch.einsum("mblxd,mblyd->mblxy", q, k) * self.scale
            ...
            x = torch.einsum("mbhxy,mbhyd->mbhxd", att_mat, v)
