...
for trial_path in trials_paths:
    ...
    for i, (ang_start, ang_fin) in enumerate(angles):
        inds = np.linspace(start=ang_start*2, stop=ang_fin*2 - 1, num=angs, dtype=int)
        SUV_data_new, SUV_ind_data_new , SEG_data_new = SUV_data[inds, :, :], SUV_ind_data[inds, :, :], SEG_data[inds, :, :]
        if Path.exists(HGUO_PATH):
            HGUO_data_new = HGUO_data[inds, :, :]
        trial_path_new = str(trial_path).replace('MIPs720', f'MIPs batches/MIPs{ang_start}_{ang_fin}')
