...
class Tester:
    ...
    def load_metrics(self):
        ...
        self.metrics_values = {set:{k: [] for k in self.metrics_obj.metrics} for set in self.args.sets}
    ...
    def compute_metrics(self, y, y_pred):
        ...
        for metricName, metricFunc in self.metrics_obj.metrics.items():
            self.metrics_values['test'][metricName].append(np.array(metricFunc(y_pred=y_pred, y=y)))

    def compute_mean_metrics(self):
        for metricName, metricFunc in self.metrics_obj.metrics.items():
            self.metrics_values['test'][metricName] = np.mean(self.metrics_values['test'][metricName])
    ...
    def test_loop(self):
        ...
        # Load wegihts
        load_weights(self.args, self.model)
